...
OK   половинная/двойная сетка не нужна: базовая точнее (сила ударов под ней выше)
INFO точность сетки: медиана отклонения реальных ударов от узлов 3.1 мс
INFO сильных ударов в сетке (±17 мс = полкадра @ 30 fps): 61/61 = 100%
...
номер бита — арифметикой, а не позицией в массиве, затем два прохода отсева
выбросов и МНК по инлайнерам. Кадры считаются **один раз и в самом конце**:
округление каждого шага до целого кадра даёт дрейф до 38 кадров к концу
двухминутного трека.

Важное, что вскрылось на замерах: **фазу нельзя брать из бит-трекера**.
`beat_track` систематически отстаёт от реального удара на 16–34 мс, и лаг не
уходит при уменьшении hop — это лаг алгоритма, а не квантование. У пика
onset-огибающей смещение пропорционально hop. Монтаж по такой фазе звучит
«почти в такт», то есть ровно тот дефект, из-за которого весь метод и нужен.
Фаза ищется несмещённо — сканом по максимуму энергии: ошибка ≤3 мс.

Подробно — [references/beat-sync.md](references/beat-sync.md).

## Проверено на реальном продукте

В `demo/` лежит собранный ролик: **живые скриншоты работающего продукта**,
смонтированные этой сеткой. Не макет, не анимация «по мотивам».

| Проверка | Результат |
|---|---|
| Склейки против узлов сетки | **0 кадров** расхождения (5/5 шотов) |
| Точность сетки на реальном треке | медиана **3.1 мс** при полкадре 16.7 мс |
| Сетка против известной истины | кадры 0, 15, 30, 45… — совпадают покадрово |
| Покрытие сильных ударов | **100%** |

Воспроизвести целиком: `demo/README.md`. Там же — четыре дефекта, которые демо
вскрыло (дефолтная фаза трекера, затухание перед склейкой, текст мимо сетки,
отсутствующий `Easing.quart` в Remotion 4.0.484).
...
│   ├── cards-index.json            ← то же машиночитаемо (_ru — перевод, _zh — оригинал)
...
├── demo/                           ← собранный ролик на реальном продукте
│   ├── README.md                   ← как воспроизвести и что демо вскрыло
│   ├── make_bgm.py                 ← BGM с известной истиной (проверка точности)
│   ├── check_cuts.py               ← проверка склейек в готовом mp4
│   ├── comp/                       ← Remotion: приёмы из карточек
│   ├── shots/                      ← живые скриншоты продукта
│   └── out/demo.mp4 · out/grid.json ← артефакты прогона (17 с, 1600×900)
├── scripts/
